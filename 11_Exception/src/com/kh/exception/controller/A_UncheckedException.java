...
public class A_UncheckedException {
	...
	/*
	 * UncheckedException
	 *  - RuntimeException처럼 프로그램 실행시 발생되는 예외들이 대표적인UncheckedException
	 *    계열의 예외들이다.
	 *  - RuntimeException의 자식 클래스들
	 *    1. ArrayIndexOutOfBoundsException
	 *    2. NegativeArraySizeException : 배열의 사이즈가 음수일때 발생되는 에러
	 *    3. ClassCastException
	 *       - 허용할 수 없는 형변환이 진행될 경우 발생하는 예외
	 *       - ex) p1이 Child2객체로 생성된 참조변수일 때,
	 *             (Child1) p1 등을 수행하는 경우
	 *    4. NullPointerException   (null을 통해 실행할수 없는 코드를 수행할때)
	 ...
	 *       즉, 굳이 예외처리(예외처리코드)를 할 필요가 없다는 의미
	 *    
	 * */
	...
	public void method1() {
		...
		// 예외처리 방법 1) 조건문으로 애초에 예외가 발생하지 않게끔 처리한다.(사실 예외처리 방법은 아님)
//		if(num2 == 0) {
...
//		}
	...
	public void method3() {
//		ArrayIndexOutOfBoundsException : 배열의 인덱스 값을 벗어나는 경우
//		NegativeArraySizeException : 배열의 크기지정에 서 음수를 받을때
		
		System.out.println("배열의 크기 : ");
		int size = sc.nextInt();
		
		// 예외처리 방법 1. 다중 catch 블럭 // 길어지면 보기 힘듬
//		try {
//			int [] arr = new int[size];
//			System.out.println("100번째 인덱스의 값 : " + arr[100]);
//		}catch(ArrayIndexOutOfBoundsException e) {
//			System.out.println("배열의 인덱스 값을 벗어나는 경우");
//		}catch(NegativeArraySizeException e) {
//			System.out.println("배열의 크기지정에 서 음수를 받을때");
//		}
		
		// 처리방법 2. 다형성을 이용한 catch // 무슨에러 인지 알수 없음
		try {
			int [] arr = new int[size];
			System.out.println("100번째 인덱스의 값 : " + arr[100]);
			
			//다중 catch블럭 자성시 범위가 작은 자식클래스를 먼저 기술해야 한다.
		}catch(ArrayIndexOutOfBoundsException e) {
			System.out.println("잘못된 인덱스 접근입니다.");
		}
		catch(RuntimeException e) { // 다형성 적용
			System.out.println("예외가 발생하였습니다."); // 정확히 어떤 예외가 발생했는지 파악 불가
			e.printStackTrace();
		}
	}
