...
public class MethodTest2 {
	 ...
	 * 
	 * */
	static int count = 1; //static, 생성시점 프로그램 시작과 동시
	int count2 = 2;// heap, 객체 생성시
	...
	public static int method(int num1) {
		 ...
		 *  - 변수의 생성시점이 다르기 때문
		 *  - static메서드는 프로그램 시작과 동시에 정적메모리영역에 할당되는데,
		 *    같은 시점에 static변수 함께 할당되기때문에 static변수는 사용가능.
		 *  - 하지만, 일반 전역변수는 객체 생성시점에 메모리에 공간이 할당되므로,
		 *    static메서드에서 사용이 불가능하다.
		 * */
		// 매개변수 * static변수(문제없음)
		return num1 * count;
//		return num1 * count2;//12시 30분
