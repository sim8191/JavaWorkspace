...
public class NetworkTest {
	 ...
	 *  - InetAddress(자바에서 IP주소글 가지고 있는 클래스)
	 * 
	 * */
	...
	public void test1() {
		...
		try {
			...
			InetAddress[] arr2 = InetAddress.getAllByName("naver.com");
	...
	public void test2() {
		/* 자원 : 웹툰 자원, 쇼핑 자원 이런 느낌
		 * URL
		 *  - 인터넷상에 존재하는 서버들의 자원에 접근할 수 있는 주소.
		 *  - 자바에서는 URL클래스를 통해 URL자원을 다룰 수 있다.
		 *  - 실제 URL자운에 대한 객체를 생성하여 자원에 연결하고 데이터를 읽어올 수 있다.
		 *  
		 *  URL구조
		 *   - https://kh-academy.co.kr:433/work/boardView.kh?no=61575&cpage=1&classify=&boardflag=
		 * 
		 * protocol : 통신규약(http, https, smtp, file, .....)
		 * hostname : 도메인이름(ip주소) kh-academy.co.kr
		 * port : 서비스 번호. 호스트내에 특정 서비스를 가리키는 논리적인 번호.
		 *        포트번호 별로 다양한 서비스를 제공할 수 있다.
		 *        각 프로토콜의 기본포트는 생략이 가능한다.
		 *        https의 기본포트번호 443, http의 기본포트 : 80, ftp의 기본포트 : 20
		 * 자원경로(path) : hostname과 port내부에 자원이 존재하는 세부 경로 
		 *               /work/boardView
		 * querystring : 자원을 얻기 위한 서버에 전달해야하는 값들
		 *               ?no=61575&cpage=1&.....
		 * 
		 * */
		try {
			URL url = new URL("https://kh-academy.co.kr:433/work/boardView.kh?no=61575&cpage=1&classify=&boardflag=");
