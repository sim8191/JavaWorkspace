...
public class Run {
	 ...
	 *  - 클라이언트 서버간에 연결이 되어 있으므로 실시간으로 데이터 송수신이 가능.
	 * 
	 * Socket
	...
	public static void main(String[] args) {
		...
		try {
			...
			while(true) {
				...
				System.out.println("클리아언트 ip주소 : "
						+ client.getInetAddress().getHostAddress());
				
				// 4) 연결된 클라이언트와 통신할 수 있는 입출력 스트림 생성.
				try(BufferedReader br = new BufferedReader(
						new InputStreamReader(client.getInputStream()));
					PrintWriter pw = new PrintWriter(client.getOutputStream(), true);//있어도 그만 없어도 그만(오토 플러시 설정)
					){
					while(true) {
