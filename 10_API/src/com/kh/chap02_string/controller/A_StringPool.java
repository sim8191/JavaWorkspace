...
public class A_StringPool {
	...
	public void method1() {
		...
		//toString이 오버라이딩 되어있기 때문에 문자열이 뜸
	...
	public void method2() {
		 ...
		 * 
		 * 문자열 상수풀 동작방식(jvm)
		 * 1. 프로그램 실행 도중 문자열 리털럴을 만나면, 우선 문자열 리터럴 값이 이미
		 *    상수불에 보관되어 있나 검사를 진행(intern)
		 *    
		 * 2. 만약 상수풀에 저장된 리터럴값이 아니라면, 문자열의 hashCode함수를 호출하고
		 *    그 값을 상수풀의 주소값으로 활용하여 값을 저장한다.
		 *    
		 * 3. 만약 이미 상수풀에 그 값이 저장되어 있다면, 저장되어있는 주소값을 반환한다.(얕은복사)
		 * 
		 * */
		 ...
		 *    하고나 데이터가 없었다면 상수풀에 새롭게 값을 등록하고 그 주소값을 반환한다.
		 * */
