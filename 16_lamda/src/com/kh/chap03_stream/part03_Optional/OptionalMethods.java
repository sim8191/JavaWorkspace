...
public class OptionalMethods {
	 ...
	 *  - Optional을 객체로 사용할 때 Optional내부에 데이터가 있는지 없는지 항상확인하는 절차를 강
	 *    함으로써 에러를 방지한다.
	 *  - Optional객체를 변수에 저장하여 관리하는 경우, 변수에 maybe, opt와 같은 Optional타입을
	 *    암묵적으로 제시한다.
	 * */
	...
	public static void main(String[] args) {
		 ...
		 *  - orElseGet(Supplier) : get()으로 꺼낼 값이 존재하지 않을 경우 람다식을 통해 구현한
		 *    값을 반환
		 *  - orElseThrow(Supplier) : get()으로 꺼낼값이 없을때, 람다식을 통해 구현한 에로객체를
		 *    호출
		 * 
		 * */
	...
	public void test1() {
		List<String> list = Arrays.asList("홍길동","신사",null,"1가나다","");
		list.stream()
...
//				.orElse(0);
	...
	public String getEmailOfMemberFromOrder(Order order) {
		// optional없이 최대한 안전(NPE가 발생하지 않게)하게 email추출하기
//		if(order != null) {
//			Member member = order.getMember();
//			if(member != null) {//order.setMember(member);//여기가 꼭필효한것을 아니기때문에 여기를 작성하는거
