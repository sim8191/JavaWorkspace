...
public class Account {
	 ...
	 * 
	 * 동기화 처리 방법
	 * 1. synchronized 메서드 만들기(비권장)
	 *  - 메서드 전체에 대해 락을 걸고, 메서드 종료시 락이 풀린다.
	 * 
	 * 2. synchronized block 사용하기.(권장)
	 *  - 동기화가 필요한 일부 내용에만 락을건다.
	 *  - 동기화 브럭이 끝나는 순간 락이 풀린다.
	 *  - 임계영역이 아닌 지역까지는 모든스레드가 접근 가능하기 때문에, 메서드 전체로 임계영역으로 지정하는것보다.
	 *    효율이 좋다.
	 * */
	...
	public /* synchronized */ void withdraw(int money) {
		...
		synchronized (this) {//동기화 블럭
			...
			if(money <= balance) {
				balance -= money;
				System.out.printf("[%s] %d원 출금 >>> 잔액 : %d",name,money,balance);
			}else {
