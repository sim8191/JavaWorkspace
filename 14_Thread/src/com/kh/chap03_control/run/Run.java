...
public class Run {
	...
	public void test1() {
		 ...
		 *     
		 *  3) join(ms)
		 *   - 지정된 시간동안 다른 스레드를 실행시키고, 현제 스레드는 대기시키는 메서드
	...
	public void test2() {
		 ...
		 *  1) notify() / notifyAll()
		 *   - 동기화된 블럭안에서만 사용하여, 대기중인 스레드를 깨우는(Runnable) 메서드
		 *  2) interrupt()
		 *   - sleep이나, join에 의해 이시정지된 메서드를 깨우는 역활의 메서드로,
		 *     InterruptedException을 강제로 발생시킨다.
		 * */
		...
		while(true) {
			...
			if(str.equals("exit")) {
				cnt.interrupt();//일시정지상태인 스레드를 께우는 용도
				break;
			}
		}
	}
