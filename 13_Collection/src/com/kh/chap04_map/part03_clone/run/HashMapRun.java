...
public class HashMapRun <K , V> {
	...
	public int getIndex(K key) {
		 ...
		 * 77777778 % 10 == 8번 인덱스
		 *  */		
		return Math.abs(hash) % capacity;//(여기서의 저장공간은 capacity)
	}
...
//				table[index].next = new Entry<>(key,value,null);//잘못된 예시
	...
	public boolean containsKey(K key) {
		...
		do {
			...
			entry = entry.next;//엔트리에 엔트리의 넥스트 값을 넣어라
		}while(entry != null);
		...
		do {
			if(entry.key.equals(key)) {
				...
				if(table[index] == entry) {
					...
					// 두번째 이상인 entry인 경우 next필드의 값을 null로 변경하여 삭제
