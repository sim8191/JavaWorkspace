...
public class ListRun {
	 ...
	 *  - 효율적인 기능들이 정의되어있는 틀
	 *  
	 * 데이터들이 새롭게 추가되거나 삭제가되거나, 수정이 되는 기능들을 이미 정의해놓은 틀이다.(Collection 한줄평)
	 * 
	 * 배열과 컬랙션(List)의 차이점
	 *  - 배열의 단점
	 *  1. 같은자료형의 데이터만 저장가능
	 *  2. 배열의 크기를 수정할 수 없다.
	 *  3. 배열내부의 값들을 조작하기 불편하다.(할수는 있음(배열순차정리 이런거 근대 하기 힘들다고))
	 *  
	 *  - 컬랙션의 장점
	 *  1. 여러자료형의 값을 저장할 수 있다.(제네릭 설정을 통해 한가지 유형의 데이터만 저장도 가능)(사실 장점이자 단점임)
	 ...
	 *   - 중복값을 허용하지 않는다.(헤시코드와 연관)
	 *   - HashSet, TreeSet
	 *  
	 *  Map계열
	 *   - 담고자 하는 값(value)과 그 값을 저장하는 키(key)를 함께 저장
	 *   - 값 저장시 순서를 유지 하지 않는다.(index x)
	 *   - key값을 중복은 허용하지 않으나, value의 중복은 허용
	 *   - HashMap, Properties, ....
	 * */
	
	public static void main(String[] args) {
//		ArrayList list = new ArrayList(); //list계열중 ArrayList가 가장많이 쓰임
//		List list = new ArrayList(); //(보통많이 사용하는 방식)크기 10배열 생성
		// 경고 표시가 뜨는 이유 제네릭 설정을 안해서
		 ...
		 * 2. 컬렉션에 저장된 객체를 꺼네서 사용할때 직접 형변환하는 작업을 없애기 위해서.
		 * */
...
//		ArrayList<Music> list = new ArrayList<Music>(3);
		...
		// 만약 제네릭 설정을 안했다면?
		Music m2 = (Music) list.get(0); // 강제형변환 필요(다운케스팅)
		
//		ArrayList<int> Iarr = new ArrayList<>(3);// 안됨
		ArrayList<Integer> Iarr = new ArrayList<>(3);
		Iarr.add(1);
		Iarr.add(9);
		Iarr.add(900);
		
		//7. contains(E e) : boolean(list 안에 e의 값이 있다면 true)
		System.out.println(Iarr.contains(0));
...
//			iter.remove();// list내부의 값을 삭제하는 메서드.
		...
		// - index1에서부너 index2까지 데이터를 추출한 새로운 subList를 반환하는 메서드
...
//		list.clear();
		...
		System.out.println("=========");
		// 13. Collections.sort(List list) : 배열을 정렬해주는 메서드(중요함)
		 ...
		 *  - VO클래스 이되 별도 클래스에 Comparator를 상속시켜서 구현한다.
		 *  - 여러개의 정렬조건을 만들 수 있다.
		 * 
		 * */
		System.out.println("=============");
		...
		Collections.sort(list, comp2); // comp2는 추가 정렬조건을 객체화 해서 넣논것
